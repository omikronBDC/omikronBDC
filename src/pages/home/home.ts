...
export class HomePage {
  ...
  sales(item: any) {
    ...
    modal.onDidDismiss((data) => {
      ...
      if (data) {
        this.get_upcoming_visit_list();
        this.event.publish('opencreatetab', item)
        this.get_calendar_data();
      }
    })
    // alert('Coming Soon');
  ...
  get_upcoming_visit_list() {
    ...
    this.rest_api.postData_withoutLoder(Data, 'get_upcoming_visite_list').then((res: any) => {
      ...
      this.offline.updateUpcomingList(this.clients);
      // console.log('res---------------',res);
    }).catch((err: any) => {
      // console.log('err---------------',err);
      this.auth.isDisconnect = true;
      this.offline.getUpcoming().then((res) => {
        this.clients = res;
      })
      this.typ = '';
      console.log('this.clientsiiiiiiiiiiiiiiiiiiiiiiiiiiiii', this.clients);
    })
  }
  ...
  filter(filter: string) {
    ...
    if (filter == 'Week') {
      ...
      let d = new Date();
      let m = new Date().toISOString().split('T')[0];

      let objToSend = d.getFullYear() + '-' + d.getMonth() + '-' + d.getDate();
      let obj = { "year": d.getFullYear(), "month": d.getMonth(), "date": d.getDate() };
      calArray.push(obj);
      //  / this.visitEvents = calArray;
      let Data = {
        employee_id: { "value": this.auth.getCurrentUserId(), "type": "NO" },
        filter: { "value": m, "type": "NO" },
      }
      this.rest_api.postData_withoutLoder(Data, 'get_upcoming_visite_list').then((res: any) => {
        console.log(res);
        if (res.status == 1) {
  ...
  getWeek() {
    ...
    let arrToSend: any = [];
    for (let index = 0; index < this.dateselected.length; index++) {
      let date = this.dateselected[index].year + "-" + (this.dateselected[index].month + 1) + "-" + this.dateselected[index].date;
      let fdate = moment(date, 'YYYY-M-DD').format("YYYY-MM-DD");

      arrToSend.push(fdate);
    }
    console.log('check this array to send', arrToSend)
    //  this.visitEvents = calArray;

    // console.log(calArray)

    // for (let index = 0; index < calArray.length; index++) {
    //   arrToSend.push(this.formatDate(calArray[index]))
    // }
    let Data = {
      employee_id: { "value": this.auth.getCurrentUserId(), "type": "NO" },
